        ...
        <div class="col-md-3">
            <div id="div_status" class="form-group">
                    ...
                    @php $emp_status=collect($status)->where('emp_id',$employee->emp_id)->first();@endphp
                        <label class="radio-inline btn btn-icon btn-danger" for="inactive"> <input type="radio" name="status" id="inactive" checked value="inactive" {{$emp_status->emp_status=='inactive' ? 'checked' : ''}}><i class="fa fa-times"></i></label>
                        <label class="radio-inline btn btn-icon btn-success" for="active"> <input type="radio" name="status"  id="active" value="active" {{$emp_status->emp_status=='active' ? 'checked' : ''}}><i class="fa fa-check"></i></label>
                    </div>
                </div>
            </div>
        </div>
